  ...
  const uploadAndDetect = () => {
    ...
    listImageBase64.length
      ? listImageBase64.forEach((item) => {
          axios.post("https://web-production-1fd01.up.railway.app/detect", { base64: item }).then(function (response) {
    ...
    <>
      <div className="main-upload" id="main-upload" style={{ backgroundColor: darkMode && "lightslategrey" }}>
        ...
        <div className="box-upload">
          <DropFileInput onFileChange={fileChangeHandle} isDetected={isDetected} />
          {loading ? (
            <>
              <Skeleton
                active
            ...
            <>
              {isError && (
                <p style={{ marginTop: "50px", fontSize: "24px", color: "red", fontWeight: "bold" }}>
                  No object detected
                </p>
              )}
              <button className="button i-button" onClick={uploadAndDetect}>
                Upload and detect
              </button>
            </>
          )}
